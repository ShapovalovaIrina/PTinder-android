...
public class PutRequest extends AsyncTask<PostRequestParams, Void, String> {
    @Override
    protected String doInBackground(PostRequestParams... postRequestParams) {
        System.out.println("Put Request on url " + postRequestParams[0].getUrl());
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.code() == 200) {
                ...
                Log.d("Put request", "Have request answer with code " + response.code());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
